...
class Main(tk.Frame):
    ...
    def delete_records(self):
        for selaction_item in self.tree.selection():
            self.db.c.execute("""DELETE FROM db WHERE
                              id = ?""", (self.tree.set(selaction_item, "#1"), ))
            self.db.conn.commit()
            self.view_records()
    ...
    # функция вызова окна редактирования
    def open_update_dialog(self):
        Update()
    

    # функция вызова окна поиска
    def open_search_dialog(self):
        Search()


    # функция хранения и инициализации объектов
    def init_main(self):
